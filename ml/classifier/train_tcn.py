...
def get_tcn_model(
    input_vector_size, target_vector_size=1, num_filters=48, learning_rate=0.002
):
    model_input = Input(shape=(None, input_vector_size))
    model_output = TCN(return_sequences=False, nb_filters=int(num_filters))(model_input)
    model_output = Dense(target_vector_size, activation="sigmoid")(model_output)

    model = Model(inputs=[model_input], outputs=[model_output])
    model.compile(optimizer=Adam(lr=learning_rate, clipnorm=1.0), loss="binary_crossentropy",
